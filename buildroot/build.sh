...
if [ -f "${BUILDROOT_IMAGE_OUT}" ]; then
	log_blue "Buildroot produced a new image!"
	if [ -f "${SCRIPT_DIR}/hudOS.img" ]; then
		rm "${SCRIPT_DIR}/hudOS.img"
	fi
	mv "${BUILDROOT_IMAGE_OUT}" "${SCRIPT_DIR}/hudOS.img"
fi

if [ "${BUILDROOT_DIR}/.config" -nt "${SCRIPT_DIR}/br2_hudos/configs/hudos_defconfig" ]; then
...
BUILDROOT_DEPLOY_KEY_OUT="${BUILDROOT_DIR}/output/build/hudOS-deploy-user-0.0.1/deploy_ed25519"
if [ "${BUILDROOT_DEPLOY_KEY_OUT}" -nt "${SCRIPT_DIR}/deploy_ed25519" ]; then
