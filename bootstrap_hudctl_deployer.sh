...
jq -nM \
    ...
    '[{ "name": $app_name, "manifest": $app_manifest, "linked": false }]' \
    > "${HUD_APPS_CATALOG}"
...
ln -sf "${HUDCTL_APP_DIR}/binaries/hudctl-link" "${HUD_BIN_DIR}/hudctl-link"
...
PATH="${HUD_BIN_DIR}:${PATH}" hudctl link hudctl